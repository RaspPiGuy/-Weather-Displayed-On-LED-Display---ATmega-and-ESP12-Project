/*  TimesSquareScroll_Reverse_V2R1.ino   
    MJL  thepiandi.blogspot.com
    
Specific to Mike Oshinski's Big Display: one row of 18 5x7 character LED displays

This iteration displays message starting at the left column of the display.
Characters are added to the left and the displays scrolls right one column
...
Version1:
Revision 1:  10/22/2015

Version2:
Revision 1:  7/19/2016 - removing switch

This is based upon the TimesSquareScroll_V4R1:
...
void setup() {
  ...
  while(1){    
    ...
    lastByte = false;
    endOfMessage = charAddress + noOfChars;

    do{  //displays the entire message
      charAddress++;
      if (charAddress >= endOfMessage){
        lastByte = true;
      }
      revChar = ReadSerialEEPROM(charAddress);
      if (((revChar > 47 && revChar < 58) || (revChar > 64 && revChar < 91) 
      || (revChar > 96 && revChar < 123 ) || (revChar == 39) || (revChar == 45)) && (!lastByte)){
        i = 0;
        do{
          revChar = ReadSerialEEPROM(charAddress + 1 + i);       
          i++;
        } while (((revChar > 47 && revChar < 58) || (revChar > 64 && revChar < 91) 
        || (revChar > 96 && revChar < 123 ) || (revChar == 39)|| (revChar == 45)) && (!lastByte));
      ...
      k = 0;
      do{  //displays a single character or a word
        for (charIndex = 0; charIndex < 6; charIndex++){
          if (charIndex == 0){
            charToDisplay = ReadSerialEEPROM(charAddress - k);
          }
          ShiftRowData();
          if (charIndex != 5){
            rowData[0] = (eeprom.EEPROM_read((5 * charToDisplay) + (4 - charIndex))) ^ 0x7f;
          }
          else{
    ...
    //we have finished displaying the message now to display a blank display
    for (i = 0; i < 6* gap; i++){
      ShiftRowData();
      rowData[0] = 0;
      for (j = 0; j < displayCount; j++){
